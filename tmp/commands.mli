...
module Kojienv: sig
  val new_ : Koji.target -> command

  val clean: Koji.environment -> command

  val configure : Koji.target -> spec: Repo.SpecSCM.t -> command

  val run : Koji.target -> string list -> spec:Repo.SpecSCM.t -> command

  val scratch : ?clean:bool -> ?remote:bool -> Koji.target -> spec:Repo.SpecSCM.t -> command

  val apush : spec:Repo.SpecSCM.t -> command

  val build:  Koji.target -> spec_repo:Repo.spec -> command

  val installdeps: specfile:string -> Koji.target -> spec_repo:Repo.spec -> command

  val install : packages:package list -> Koji.target -> spec_repo:Repo.spec -> command

  val update: packages:package list -> Koji.target -> spec_repo:Repo.spec -> command

  val mockop: string list -> Koji.target -> spec_repo:Repo.spec -> command
end
...
module Kapi: sig
  ...
  val tag_of_suffix: username -> suffix -> Koji.target
  ...
  val clone_tag: source:Koji.target -> suffix -> [< `Event of int | `Repoid of int] -> command
  ...
  type what = Tag of Koji.target | Url of { url: Uri.t; branch: string; version: string }
