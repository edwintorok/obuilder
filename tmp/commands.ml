...
module Kojienv_ = struct
  let new_ (Koji_.Target tag) = {cwd= None; args= ["kojienv"; "new"; tag]}

  let clean (Koji_.Environment env) = {cwd= None; args= ["kojienv"; "clean"; env]}

  let kojienv ~spec:((Spec spec_repo, _) : Repo.SpecSCM.t) args =
    {cwd= Some spec_repo; args= "kojienv" :: args}

  let kojienv_tag cmd args (Koji_.Target tag) = kojienv (cmd :: tag :: args)
  ...
  let kojienv_tag' cmd args (Koji_.Target tag) ~spec_repo:(Repo.Spec spec) =
...
module Kapi = struct
  ...
  let tag_of_suffix (Username username) (Suffix suffix) =
    Koji_.Target (Printf.sprintf "pb-%s-%s" username suffix)
  ...
  let clone_tag ~source:(Koji_.Target tag) suffix source =
    let source =
      match source with
      | `Event id ->
