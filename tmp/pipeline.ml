...
let rpms_union (RPMs a) (RPMs b) = RPMs (Astring.String.Set.union a b)

let rpms_union_all lst =
  List.fold_left rpms_union (RPMs Astring.String.Set.empty) lst
...
let build_requires_external ~provides:(BuildRequires provides)
  ...
  let build_requires_names_only =
    Astring.String.Set.map maybe_pkg_name build_requires
  in
  let required_names_only =
    Astring.String.Set.diff build_requires_names_only provides
  in
  ...
  *)
  BuildRequires
    (Astring.String.Set.filter
       (fun build_requires ->
         Astring.String.Set.mem
           (maybe_pkg_name build_requires)
           required_names_only )
       build_requires )
...
let koji_pipeline_rpm ~provides (on, kojitag, builds, srpm_env) (spec, srpm) =
  ...
  let rpmbuild_env =
    build_requires |> Current.gate ~on |> Steps.kojitag_new_or_update builds
  in
...
let analyze_spec srpm_env spec =
  ...
  let provides_to_srpm_build =
    provides |> String.Set.to_seq
    |> Seq.map (fun p -> (maybe_pkg_name p, [name]))
    |> List.of_seq
  and srpm_to_build_requires =
    (name, requires |> String.Set.elements |> List.map maybe_pkg_name)
  in
...
let sorted_dependencies srpm_env specs =
  let open Current.Syntax in
  let+ names_and_deps =
    specs |> List.map (analyze_spec srpm_env) |> Current.list_seq
  in
  ...
  |> List.iter
     @@ fun deplist ->
     Logs.info (fun m ->
         m "Deps: %a" Fmt.(Dump.pair string (Dump.list string)) deplist ) ) ;
  match Tsort.sort deps with
  | Tsort.ErrorCycle cycle ->
      Fmt.invalid_arg "Dependency cycle between .spec: %a"
  ...
  | Tsort.Sorted order ->
      let result =
        ...
        |> List.filter_map (fun dep ->
               Astring.String.Map.find_opt dep path_to_spec
               |> Option.map @@ fun spec -> (dep, spec) )
      in
      Logs.debug (fun m ->
          m "dependencies: %a" Fmt.(Dump.list @@ using fst string) result ) ;
      ...
      let depmap = Astring.String.Map.of_list deps in
      (depmap, result)

module SpecMap = Map.Make (Tree)

let rec lookup_deps depmap rpms_map path aux =
  match Astring.String.Map.find_opt path depmap with
  | None ->
      aux
  | Some deps ->
      deps
      |> List.fold_left
           (fun aux dep ->
             match Astring.String.Map.find_opt dep rpms_map with
             | None ->
                 lookup_deps depmap rpms_map dep aux
             | Some rpm ->
                 rpm :: aux )
           aux

let build_rpms ~provides ((_, _, _, srpm_env) as env) depmap_and_sorted_deps
    codes_specs =
  let codes, (specs : spec_tree Current.t list) = List.split codes_specs in
  let open Current.Syntax in
  Current.component "dep-build"
  |>
  let** depmap, spec_order = depmap_and_sorted_deps
  and* specs = Current.list_seq specs in
  let spec_to_code =
    List.combine specs codes |> List.to_seq
    |> Seq.map (fun (Spec k, v) -> (k, v))
    |> SpecMap.of_seq
  in
  let rpms =
    spec_order
    |> List.fold_left
         (fun rpms (path, spec) ->
           let (Spec spec_) = spec in
           let code = SpecMap.find spec_ spec_to_code in
           let spec_tree = Current.return spec in
           let srpms = Steps.srpm srpm_env spec_tree code in
           let on =
             lookup_deps depmap rpms path []
             |> List.map Current.ignore_value
             |> Current.all
           in
           let new_rpms =
             koji_pipeline_rpm ~provides env (spec_tree, Current.gate ~on srpms)
           in
           Astring.String.Map.add path new_rpms rpms )
         Astring.String.Map.empty
    |> Astring.String.Map.to_seq |> Seq.map snd |> List.of_seq
  in
  let+ rpm_lists = Current.list_seq rpms in
  rpms_union_all rpm_lists

(* TODO: we can copy rpms from the resultdir/localrepo to another localrepo and createrepo *)
...
let repo_pipeline dirs =
  ...
  let _codes, specs = codes_specs |> List.split in
  ...
  let provides =
    ...
    |> Current.gate ~on
  in
  let depmap_and_sorted_deps = sorted_dependencies srpm_env specs in
  build_rpms ~provides env depmap_and_sorted_deps codes_specs
